package eu.tintera.background.tasks.handlers

import eu.tintera.background.tasks.*
import eu.tintera.background.tasks.MainViewModel.Companion.DEFAULT_TAG
...
fun testTaskRequest(
    ...
    tags = tags {
        label(DEFAULT_TAG)
        tag(
            TestTypedTag(number = Random.nextInt(0, 1000))
        )
        tag(
            TestTypedTag(number = Random.nextInt(0, 1000))
        )
