...
internal object Migration9to10 : Migration(9, 10) {
    ...
    override suspend fun migrate(connection: SQLiteConnection) {
        ...
        connection.execute("ALTER TABLE `Task_new` RENAME TO `Task`")

        // 5. Znovuvytvoření indexů POUZE pro tabulku Task
        // (Indexy pro TaskParentTask přežily, protože jsme tu tabulku nedropnuli)
        connection.execute("CREATE INDEX IF NOT EXISTS `index_Task_state_processTime` ON `Task` (`state`, `processTime`)")
        connection.execute("CREATE INDEX IF NOT EXISTS `index_Task_uniqueName` ON `Task` (`uniqueName`)")
    }
}
